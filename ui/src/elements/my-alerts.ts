...
export class MyAlerts extends LitElement {
  ...
  message(alert: Alert<EventAlert>, event: EventWithStatus) {
    if (alert.alert.update.type === 'event_was_cancelled')
      return msg(str`Event ${event.currentEvent.entry.title} was cancelled`);
  }

  renderAlert(alert: Alert<EventAlert>, event: EventWithStatus) {
    return html`<sl-card>
      <span>${this.message(alert, event)}</span>
    </sl-card>`;
  }
