...
export class GatherStore {
  ...
  ): Promise<Readable<EntryState<Event> | undefined>> {
  ...
  async updateEvent(
    ...
    const updateRecord = await this.client.updateEvent(
      originalEventHash,
      previousEventHash,
      updatedEvent
    );
  ...
  private _attendeesAttestations: Writable<RecordBag<AttendeesAttestation>> =
    writable(new RecordBag());

  private _attendeesAttestationsForAteendee: Writable<
    AgentPubKeyMap<Array<ActionHash>>
  > = writable(new AgentPubKeyMap());

  private _attendeesAttestationsForEvent: Writable<
    ActionHashMap<Array<ActionHash>>
  > = writable(new ActionHashMap());

  async createAttendeesAttestation(
    attendeesAttestation: AttendeesAttestation
  ): Promise<Record> {
    const record = await this.client.createAttendeesAttestation(
  ...
  async fetchAttendeesAttestation(
    attendeesAttestationHash: ActionHash
  ): Promise<Readable<EntryState<AttendeesAttestation> | undefined>> {
    const record = await this.client.getAttendeesAttestation(
      attendeesAttestationHash
    );
  ...
  ): Promise<EntryRecord<AttendeesAttestation>> {
    const updateRecord = await this.client.updateAttendeesAttestation(
      previousAttendeesAttestationHash,
  ...
  async fetchAttendeesAttestationsForAteendee(
    ateendee: AgentPubKey
  ): Promise<Readable<Array<ActionHash>>> {
    const hashes = await this.client.getAttendeesAttestationsForAteendee(
      ateendee
    );
  ...
  async fetchAttendeesAttestationsForEvent(
    ...
    const hashes = await this.client.getAttendeesAttestationsForEvent(
      eventHash
    );
