...
import { AgentAvatar, ProfilesStore, profilesStoreContext } from '@holochain-open-dev/profiles';
  ...
  renderSummary(entryRecord: EntryRecord<Event>) {
    return html`
      ...
      entryRecord.entry.start_time / 1000
          ...
          ${this._fetchAttendees.status === TaskStatus.COMPLETE ? html`<div class="row" style="align-items: center;">
            <span style="margin-right: 8px;">Attendees</span>
            <div class="avatar-group">
              ${this._fetchAttendees.value?.map(a => html`<agent-avatar .agentPubKey=${a}></agent-avatar>`)}
            </div>
          </div>` : html``}    
        ...
        <show-image style="width: 200px; height: 200px; flex: 0" .imageHash=${entryRecord.entry.image}></show-image>
