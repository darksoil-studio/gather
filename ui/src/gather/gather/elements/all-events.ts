...
export class AllEvents extends ScopedElementsMixin(LitElement) {
  ...
  _fetchAllEvents = new TaskSubscriber(this, 
  ...
  renderList(hashes: Array<ActionHash>) {
    ...
    return html`
      <div style="display: flex; flex-direction: column">
        ${hashes.map(hash => 
          html`<event-summary .eventHash=${hash} style="margin-bottom: 16px;"@click=${() => this.dispatchEvent(new CustomEvent('event-selected', {
          composed: true,
          bubbles: true,
          detail: {
            eventHash: hash
