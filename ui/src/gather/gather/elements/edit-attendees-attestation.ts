...
import { InstalledCell, ActionHash, AgentPubKey, Record, AppWebsocket, InstalledAppInfo } from '@holochain/client';
...
export class EditAttendeesAttestation extends LitElement {
  ...
  @state()
  _ateendees: Array<AgentPubKey | undefined> = this.currentRecord.entry.ateendees;
  ...
  async updateAttendeesAttestation() {
    ...
    try {
      ...
      this.dispatchEvent(new CustomEvent('attendees-attestation-updated', {
        composed: true,
        bubbles: true,
        detail: {
          originalAttendeesAttestationHash: this.originalAttendeesAttestationHash,
