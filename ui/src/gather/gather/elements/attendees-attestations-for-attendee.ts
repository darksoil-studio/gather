...
export class AttendeesAttestationsForAteendee extends ScopedElementsMixin(
  LitElement
) {
  ...
  _fetchAttendeesAttestations = new TaskSubscriber(
    this,
    ([store, ateendee]) =>
      store.fetchAttendeesAttestationsForAteendee(ateendee),
  ...
  renderList(hashes: Array<ActionHash>) {
    if (hashes.length === 0)
      return html`<span
        >No attendees attestations found for this ateendee.</span
      >`;
