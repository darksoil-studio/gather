...
pub fn get_my_events(_: ()) -> ExternResult<Vec<Link>> {
    ...
    get_links(
        GetLinksInputBuilder::try_new(agent_pub_key.agent_initial_pubkey, LinkTypes::MyEvents)?
            .build(),
    )
}
...
pub fn remove_from_my_events(event_or_proposal_hash: ActionHash) -> ExternResult<()> {
    let my_pub_key = agent_info()?.agent_initial_pubkey;
    let links = get_links(GetLinksInputBuilder::try_new(my_pub_key, LinkTypes::MyEvents)?.build())?;
